...
<template>
  <UCollapsible v-if="context">
    ...
    <template #content>
      <div
        class="bg-muted mt-1.5 rounded-lg p-5 flex flex-col gap-4
        text-sm max-h-[40rem] overflow-y-auto text-default"
      >
        ...
        <div v-else>
          <div v-for="(value, key) in context" :key="key" class="flex gap-2">
            ...
            <span v-else class="whitespace-pre-line text-default">{{ value }}</span>
          </div>
        </div>
      </div>
    </template>
